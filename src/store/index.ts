...
// atoms which key of defaultBlocksAtom/customBlocksAtom which are active
export const inActiveBlocksAtom = withImmer<string[]>(
  atomWithStorage("inactive", inActiveBlocks)
);
...
export const blockValuesAtom = withImmer<BlockValuesObject>(
  atomWithStorage("values", blockValues)
);
...
export const customBlocksAtom = withImmer<BlocksObjectWithId>(
  atomWithStorage("custom-blocks", {})
);
...
export const createCustomBlockAtom = atom(
  null,
  (get, set, { name, category }) => {
  ...
  (get, set, { position, dir }: { position: number; dir: "up" | "down" }) => {
    set(activeBlocksAtom, (draft) => {
      const items = arrayMove(
        draft,
        position,
        dir === "down" ? position + 1 : position - 1
      );
      return (draft = items);
    });
  }
...
export const makeBlockActiveAtom = atom(
  null,
  (get, set, { id }: { id: string }) => {
...
export const moveMultipleBlockItemAtom = atom(
  ...
  (
    get,
    set,
    { dir, name, id }: { dir: "left" | "right"; name: string; id: string }
  ) => {
    set(blockValuesAtom, (draft) => {
      const item = draft[id];
      if (item.type === BlockType.Multiple) {
